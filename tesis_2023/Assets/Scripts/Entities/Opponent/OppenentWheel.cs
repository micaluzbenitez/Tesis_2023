    ...
    public class OppenentWheel : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (opponentAI.IsAlive())
            {
                ...
                if (leftFrontWheel) leftFrontWheel.localRotation = Quaternion.Lerp(leftFrontWheel.localRotation, endLeftRotation, Time.deltaTime);
                if (rightFrontWheel) rightFrontWheel.localRotation = Quaternion.Lerp(rightFrontWheel.localRotation, endRightRotation, Time.deltaTime);
                
                // Apply back wheels rotation (X rotation)
                if (leftBackWheel) leftBackWheel.localRotation = Quaternion.Euler(wheelsXRotation, 0, leftZRotation);
                if (rightBackWheel) rightBackWheel.localRotation = Quaternion.Euler(wheelsXRotation, 0, rightZRotation);
            }
            else
            {
                Destroy(this);
